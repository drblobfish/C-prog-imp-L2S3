...
void read_grades(char *file_name,int **tab,int *nb_stud){
    FILE *grade_file = fopen(file_name,"rb");
    if (grade_file == NULL)
    {
    ...
    fread(nb_stud,sizeof(int),1,grade_file);

    loc_tab = (int *)calloc(*nb_stud,sizeof(int));

    fread(loc_tab,sizeof(int),*nb_stud,grade_file);
...
void write_grades(char *file_name,int *tab,int nb_stud){
    FILE *grade_file = fopen(file_name,"wb");
    if (grade_file == NULL)
    {
        perror("Error while opening the file.\n");
        exit(EXIT_FAILURE);
    ...
    fwrite(&nb_stud,sizeof(int),1,grade_file);
    fwrite(tab,sizeof(int),nb_stud,grade_file);
